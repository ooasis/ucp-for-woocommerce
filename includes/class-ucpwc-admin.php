...
class UCPWC_Admin
{
    public static function register(): void
    {
        add_submenu_page(
            'woocommerce',
            __('UCP Settings', 'ucp-acp-agent-for-woocommerce'),
            __('UCP', 'ucp-acp-agent-for-woocommerce'),
    ...
    private static function handle_actions(): ?string
    {
        ...
        switch ($action) {
            case 'rotate_ucp_key':
                ...
                return __('UCP signing key rotated. The previous key stays published while platforms pick up the new one (spec recommends a 7-day grace period).', 'ucp-acp-agent-for-woocommerce');
            case 'regenerate_acp_key':
                delete_option('ucpwc_acp_api_key');
                UCPWC_Acp::api_key();
                return __('ACP API key regenerated. Update it with your ACP platform (e.g. OpenAI) — the old key stops working immediately.', 'ucp-acp-agent-for-woocommerce');
            case 'push_feed':
    ...
    public static function render(): void
    {
        ...
        ?>
        <div class="wrap">
        <h1><?php esc_html_e('UCP/ACP Agent for WooCommerce', 'ucp-acp-agent-for-woocommerce'); ?></h1>
        <?php if ($notice) : ?><div class="notice notice-success"><p><?php echo esc_html($notice); ?></p></div><?php endif; ?>

        <h2><?php esc_html_e('Status', 'ucp-acp-agent-for-woocommerce'); ?></h2>
        <table class="widefat striped" style="max-width:900px">
            <tbody>
            <tr><td><?php esc_html_e('UCP discovery profile', 'ucp-acp-agent-for-woocommerce'); ?></td>
                <td><a href="<?php echo esc_url(home_url('/.well-known/ucp')); ?>" target="_blank"><?php echo esc_html(home_url('/.well-known/ucp')); ?></a></td></tr>
            <tr><td><?php esc_html_e('UCP REST endpoint', 'ucp-acp-agent-for-woocommerce'); ?></td><td><code><?php echo esc_html(UCPWC_Profile::endpoint()); ?></code></td></tr>
            <tr><td><?php esc_html_e('ACP discovery document', 'ucp-acp-agent-for-woocommerce'); ?></td>
            ...
            <tr><td><?php esc_html_e('ACP REST endpoint', 'ucp-acp-agent-for-woocommerce'); ?></td><td><code><?php echo esc_html(untrailingslashit(rest_url('acp/v1'))); ?></code></td></tr>
            </tbody>
        </table>
            ...
            <tr>
                ...
                <td>
                    ...
                    <?php if ($retired) : ?>
                        ...
                        echo esc_html(sprintf(_n('%d retired key still published for rotation grace.', '%d retired keys still published for rotation grace.', count($retired), 'ucp-acp-agent-for-woocommerce'), count($retired)));
                        ?></p>
                    <?php endif; ?>
                    <p><button class="button" name="ucpwc_action" value="rotate_ucp_key"
                        onclick="return confirm('<?php echo esc_js(__('Rotate the UCP signing key? The current key stays published during the grace period.', 'ucp-acp-agent-for-woocommerce')); ?>');"><?php esc_html_e('Rotate key', 'ucp-acp-agent-for-woocommerce'); ?></button></p>
                </td>
            </tr>
            <tr>
                <th><?php esc_html_e('Strict signatures', 'ucp-acp-agent-for-woocommerce'); ?></th>
                <td><label><input type="checkbox" name="strict_signatures" <?php checked(get_option('ucpwc_strict_signatures'), 'yes'); ?>>
                    <?php esc_html_e('Reject unsigned UCP requests (RFC 9421 required)', 'ucp-acp-agent-for-woocommerce'); ?></label>
                    <p class="description"><?php esc_html_e('Leave off while testing — the official conformance suite sends unsigned requests.', 'ucp-acp-agent-for-woocommerce'); ?></p></td>
            </tr>
        </table>

        <h2><?php esc_html_e('ACP (OpenAI / ChatGPT)', 'ucp-acp-agent-for-woocommerce'); ?></h2>
        <table class="form-table" style="max-width:900px">
            <tr>
                <th><?php esc_html_e('API key', 'ucp-acp-agent-for-woocommerce'); ?></th>
                <td><code><?php echo esc_html(UCPWC_Acp::api_key()); ?></code>
                    <p class="description"><?php esc_html_e('Give this to your ACP platform; every ACP request must carry it as a Bearer token.', 'ucp-acp-agent-for-woocommerce'); ?></p>
                    <p><button class="button" name="ucpwc_action" value="regenerate_acp_key"
                        onclick="return confirm('<?php echo esc_js(__('Regenerate the ACP API key? The old key stops working immediately.', 'ucp-acp-agent-for-woocommerce')); ?>');"><?php esc_html_e('Regenerate', 'ucp-acp-agent-for-woocommerce'); ?></button></p></td>
            </tr>
            <tr>
                <th><label for="ucpwc_acp_webhook_url"><?php esc_html_e('Order webhook URL', 'ucp-acp-agent-for-woocommerce'); ?></label></th>
                <td><input type="url" class="regular-text" id="ucpwc_acp_webhook_url" name="ucpwc_acp_webhook_url" value="<?php echo esc_attr($field('ucpwc_acp_webhook_url')); ?>">
                    <p class="description"><?php esc_html_e('Provided by the ACP platform (out-of-band provisioning). Empty = ACP order webhooks disabled.', 'ucp-acp-agent-for-woocommerce'); ?></p></td>
            </tr>
            <tr>
        ...
        <h2><?php esc_html_e('Payments', 'ucp-acp-agent-for-woocommerce'); ?></h2>
        <table class="form-table" style="max-width:900px">
            <tr>
                <th><?php esc_html_e('Active handlers', 'ucp-acp-agent-for-woocommerce'); ?></th>
                <td>
                    <?php
                    ...
                    echo $names ? '<code>' . implode('</code>, <code>', array_map('esc_html', array_unique($names))) . '</code>'
                                : '<em>' . esc_html__('none — configure Stripe below or set a simulation secret for the mock handler', 'ucp-acp-agent-for-woocommerce') . '</em>';
                    ?>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <th><label for="ucpwc_stripe_publishable_key"><?php esc_html_e('Stripe publishable key', 'ucp-acp-agent-for-woocommerce'); ?></label></th>
                <td><input type="text" class="regular-text" id="ucpwc_stripe_publishable_key" name="ucpwc_stripe_publishable_key" value="<?php echo esc_attr($field('ucpwc_stripe_publishable_key')); ?>" autocomplete="off">
                    <p class="description"><?php esc_html_e('Advertised in the Google Pay handler config so platforms can tokenize for this store.', 'ucp-acp-agent-for-woocommerce'); ?></p></td>
            </tr>
        </table>

        <h2><?php esc_html_e('Product feed', 'ucp-acp-agent-for-woocommerce'); ?></h2>
        <table class="form-table" style="max-width:900px">
            <tr>
            ...
            <tr>
                <th><label for="ucpwc_feed_api_base"><?php esc_html_e('Feed API push (ACP)', 'ucp-acp-agent-for-woocommerce'); ?></label></th>
                <td>
                    ...
                    <p class="description">
                        ...
                        <?php if ($last = get_option('ucpwc_feed_last_push')) : ?>
                            <?php /* translators: %s: date/time of last push */ echo esc_html(sprintf(__('Last push: %s.', 'ucp-acp-agent-for-woocommerce'), $last)); ?>
                        <?php endif; ?>
                    </p>
                    <p><button class="button" name="ucpwc_action" value="push_feed"><?php esc_html_e('Push now', 'ucp-acp-agent-for-woocommerce'); ?></button></p>
                </td>
            </tr>
        </table>

        <h2><?php esc_html_e('Testing', 'ucp-acp-agent-for-woocommerce'); ?></h2>
        <table class="form-table" style="max-width:900px">
            <tr>
                <th><label for="ucpwc_simulation_secret"><?php esc_html_e('Simulation secret', 'ucp-acp-agent-for-woocommerce'); ?></label></th>
                <td><input type="text" class="regular-text" id="ucpwc_simulation_secret" name="ucpwc_simulation_secret" value="<?php echo esc_attr($field('ucpwc_simulation_secret')); ?>" autocomplete="off">
                    <p class="description"><?php esc_html_e('Enables POST /testing/simulate-shipping/{order_id} (used by the conformance suite). Empty = disabled. Never set this on a production store.', 'ucp-acp-agent-for-woocommerce'); ?></p></td>
            </tr>
        </table>

        <p><button class="button button-primary" name="ucpwc_action" value="save"><?php esc_html_e('Save settings', 'ucp-acp-agent-for-woocommerce'); ?></button></p>
        </form>
        </div>
        <?php
    }
