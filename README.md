...
- **Product feeds** (`class-ucpwc-feed.php`): one catalog mapper serving the ACP standard feed (nested Product/Variant JSON, validated against the official `schema.feed.json`; JSONL snapshot endpoint + batched `PATCH /feeds/{id}/products` push with daily cron). Token-guarded read URLs, shown on the admin screen
